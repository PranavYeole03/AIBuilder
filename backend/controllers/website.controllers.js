...
- Multi 
- column layouts become single - column on small screens
...
export const generateWebsite = async (req, res) => {
  try {
    ...
    if (user.credit < 50) {
      return res
        .status(400)
        .json({ message: "you have not enough credit to generate a website" });
    }
...
export const deploy = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: `deployed website error ${error}` });
  }
};
...
export const getBySlug = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: `get slug  error ${error}` });
