...
const LoginModel = ({ open, onClose }) => {
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      const { data } = await axios.post(`${serverUrl}/api/auth/google-login`, {
  ...
  return (
    <AnimatePresence>
      {open &&
        ...
        >
          <motion.div
            initial={{ scale: 0.80, opacity: 0, y: 60 }}
            animate={{ scale: 1, opacity: 1, y: 0 }}
            exit={{ scale: 0.9, opacity: 0, y: 40 }}
            transition={{ duration: 0.45, ease: "easeOut" }}
            className='relative w-full max-w-md p-px rounded-3xl bg-linear-to-br from-purple-500/40 via-blur-500/30 to-transparent'
            ...
            <div className='relative rounded-3xl bg-[#0b0b0b] border border-white/10 shadow-[0_30px_120px_rgba(0,0,0,0.8)] overflow-hidden'>
              ...
              <div className='relative px-8 pt-14 pb-10 text-center'>
                ...
                <h2 className='text-3xl font-semibold leading-tight mb-3 space-x-2'>
                  ...
                  <span className='bg-linear-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent'>GenAI</span>
                </h2>
                <motion.button
                  ...
                  <div className='relative flex items-center justify-center gap-3'>
                    <img src={Google} alt="Google Image" className='h-5 w-5' />
