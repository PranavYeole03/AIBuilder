...
const WebsiteEditor = () => {
  ...
  const handleDeploy = async () => {
    try {
      const result = await axios.get(`${serverUrl}/api/website/deploy/${website
        ._id}`, { withCredentials: true })
        ...
        <>
          ...
          <div className='p-3 border-t border-white/10'>
            <div className='flex gap-2'>
              <input placeholder='Describe Changes...' className='flex-1 resize-none rounded-2xl px-4 py-3 bg-white/5 border border-white/10 text-outline-none' onChange={(e) => setPrompt(e.target.value)} value={prompt} />
              <button className='px-4 py-3 rounded-2xl bg-white text-black' disabled={updateLoading} onClick={handleUpdate}><Send size={16} /></button>
            </div>
          </div>
        </>
      </aside>
      ...
      <div className='flex-1 flex flex-col'>
        <div className='h-14 px-4 flex justify-between items-center border-b border-white/10 bg-black/80'>
          <div className="flex items-center gap-2">
            <button className="p-2 rounded-lg hover:bg-white/10 transition" onClick={() => navigate("/dashboard")}><ArrowLeft size={18} /></button>
            <span className='text-sm text-zinc-400 cursor-pointer'>Live Preview</span>
          </div>
          ...
          <div className='flex gap-2'>
            ...
            <button className='p-2 lg:hidden' onClick={() => setShowChat(true)}><MessageCircle size={18} /></button>
            <button className='p-2' onClick={() => setShowCode(true)}><Code2 size={18} /></button>
            <button className='p-2' onClick={() => setShowPreview(true)}><Monitor size={18} /></button>
          </div>
      ...
      <AnimatePresence>
        {showChat && (
          <motion.div
            ...
            <button className='absolute top-4 right-4 bg-black/90 rounded-lg' onClick={() => setShowChat(false)}><X /></button>
          </motion.div>)}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showPreview && (
          <motion.div
            ...
            <button className='absolute top-4 right-4 p-2 bg-black/90 rounded-lg' onClick={() => setShowPreview(false)}><X /></button>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
